...
function dash() {
    ...
    const setUserBets=()=>{
      ...
      let list_offer=offers.split(' ')
      setUser(offers.split(' ')[0])
      console.log(offers.split(' '))
      console.log(list_offer[3])
      if (shit[3]=='Over'){
        setOverUnder(true)
      }else{
        setOverUnder(false)
      }
      setPoints(list_offer[4])
  ...
  return (
    <div className='h-screen bg-black'>
      <div class="flex items-row bg-gray-800">
        <Link href='/'>
        <header className="flex items-row bg-gray-800" >
        <h1 className='text-blue-700 font-bold text-2xl'>Basket</h1>
        <h2 className='text-white font-bold text-2xl'>Bet</h2>
        
       </header>
        </Link>
        <div class="flex items-end flex-grow justify-end mb-2"> 
          <button onClick={logOut} class="py-2 px-4 rounded-md bg-blue-700 text-white w-24 hover:bg-blue-200 font-bold">LogOut</button>
        </div>
        </div>
        <center className='flex flex-col items-center'>
          <stuff className='mt-4 flex flex-col'>
            ...
            <p className="text-gray-500 font-bold">As of June 12th 2023, the NBA season is over</p>
            <div className='bg-blue-700 rounded-md p-4 mt-3 text-white mb-3'> {matchup}</div>
            

            <div>
            <button className='bg-blue-700 text-white rounded-md p-2 mr-5' onClick={backGame}>Prev Game</button>
