...
function dash() {
    ...
    const check=()=>{
      if (homeTeamOdds==0 && awayTeamOdds==0){
        setAllow(false)
      }
    ...
    const sendData=async()=>{
      ...
      if(allow==true && betError==false && homeerror==false && awayerror==false){
    ...
    const getOdds=async()=>{
     ...
     // let list_bets=result.data["eventGroup"]["offerCategories"][0]["offerSubcategoryDescriptors"][4]["offerSubcategory"]["offers"][0][0]["outcomes"]
      for(let i=0; i<test_list.length; i++){
        ...
        let iterate_list=result.data["eventGroup"]["offerCategories"][0]["offerSubcategoryDescriptors"][4]["offerSubcategory"]["offers"][i][0]["outcomes"][0]
        let secon=result.data["eventGroup"]["offerCategories"][0]["offerSubcategoryDescriptors"][4]["offerSubcategory"]["offers"][i][0]["outcomes"][1]

        //console.log(iterate_list)

       // console.log(typeOf(iterate_list))
       //console.log(secon)
       //console.log(list_bets.length)

       // console.log(iterate_list["label"])

        for(let a=0; a<list_bets.length; a++){
          //console.log(list_bets[a])
          let outcome=list_bets[a]

          //console.log(outcome["label"])

          if(outcome["label"].includes(homeTeam)){
           if(outcome["label"].includes("Under")){
            setHomeOdds(outcome["oddsDecimal"])
           }
          }
          if(outcome["label"].includes(awayTeam)){
            if(outcome["label"].includes("Under")){
              setAwayOdds(outcome["oddsDecimal"])
            }
          }
    ...
    <div className='h-screen bg-black'>
        ...
        <center className='flex flex-col items-center'>
          <stuff className='mt-4 flex flex-col'>
            ...
            {oddsError===true?<div className='text-blue-700 font-bold'>Matchup Is Over</div>:<div></div>}
